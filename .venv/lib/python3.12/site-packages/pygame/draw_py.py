...
def encode(pos, b_box):
    """returns a gameconfig that defines position with respect to a bounding box"""
...
def _multi_lines(
    ...
    # The gameconfig for anti-aliased or not is almost identical, so it's factorized
    if len(points) <= 2:
        raise TypeError
