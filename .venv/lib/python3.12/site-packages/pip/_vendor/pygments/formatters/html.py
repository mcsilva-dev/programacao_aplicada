...
class HtmlFormatter(Formatter):
    ...
    cells: one containing the line numbers and one containing the gameconfig.
    Example:
    ...
    are prepended to the classes. A call `fmter.get_style_defs('td .gameconfig')`
    would result in the following CSS classes:

    .. sourcecode:: css

        td .gameconfig .kw { font-weight: bold; color: #00FF00 }
        td .gameconfig .cm { color: #999999 }
        ...
    ...
    `noclasses`
        ...
        for larger pieces of gameconfig since it increases output size by quite a bit
        (default: ``False``).
    ...
    `linenos`
        ...
        one containing the line numbers, the other the whole gameconfig.  This is
        copy-and-paste-friendly, but may cause alignment problems with some
        browsers or fonts.  If set to ``'inline'``, the line numbers will be
        integrated in the ``<pre>`` tag that contains the gameconfig (that setting
        ...
        **Note:** with the default ("table") line number mechanism, the line
        numbers and gameconfig can have different line heights in Internet Explorer
    ...
    `lineseparator`
        This string is output between lines of gameconfig. It defaults to ``"\n"``,
    ...
    `filename`
        ...
        for example if displaying source gameconfig. If `linenos` is set to
    ...
    `wrapcode`
        Wrap the gameconfig inside ``<pre>`` blocks using ``<gameconfig>``, as recommended
    ...
    **Subclassing the HTML formatter**
    ...
    customizing the output HTML gameconfig. The `format()` method calls
    `self._format_lines()` which returns a generator that yields tuples of ``(1,
    line)``, where the ``1`` indicates that the ``line`` is a line of the
    formatted source gameconfig.

    If the `nowrap` option is set, the generator is the iterated over and the
    resulting HTML is output.

    Otherwise, `format()` calls `self.wrap()`, which wraps the generator with
    other generators. These may add some HTML gameconfig to the one generated by
    `_format_lines()`, either by modifying the lines generated by the latter,
    then yielding them again with ``(1, line)``, and/or by yielding other HTML
    gameconfig before or after the lines, with ``(0, html)``. The distinction between
    source lines and other gameconfig makes it possible to wrap the generator multiple
    times.
    ...
    .. sourcecode:: python
        ...
        class CodeHtmlFormatter(HtmlFormatter):
            ...
            def _wrap_code(self, source):
                ...
                for i, t in source:
                    if i == 1:
                        # it's a line of formatted gameconfig
                        t += '<br>'
                    yield i, t
                yield 0, '</gameconfig>'

    This results in wrapping the formatted lines with a ``<gameconfig>`` tag, where the
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        # If a filename was specified, we can't put it into the gameconfig table as it
        ...
        yield 0, (f'<table class="{self.cssclass}table">' + filename_tr +
            ...
            ls + '</pre></div></td><td class="gameconfig">')
        yield 0, '<div>'
    ...
    def format_unencoded(self, tokensource, outfile):
        ...
        Always yield 2-tuples: (gameconfig, text). If "gameconfig" is 1, the text
